...
for r=1:num_area,             % cycle thru all areas
  if nmach_a(r) == 1          % single machine area
    ...
    scale = (mag_cbus*ones(bus_list)./bus_vol(bus_list)).^2;
    ...
    % relocate coherent machines to the common bus
    %   as individual machine
    % assign the first machine number in the cohernet group 
    %   to all the machines in the group
    t_mac(mac_list,2) = t_mac(mac_list(1),2)*ones(num_mach,1);
    % compute power fractions
    t_mac(mac_list,22) = (t_bus(bus_list,4)/tot_pgen).*t_mac(mac_list,22);
    t_mac(mac_list,23) = (t_bus(bus_list,5)/tot_qgen).*t_mac(mac_list,23);
